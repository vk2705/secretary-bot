...
### Remote-mode auth: Google OIDC (`mcp_oauth.py`)

The remote transport used to gate access with a single shared `?key=` token; it's now real OIDC login, implemented in `mcp_oauth.py`'s `GoogleOAuthProxyProvider`. `mcp_server.py` passes it into `FastMCP(auth_server_provider=..., auth=AuthSettings(...))`, which makes the MCP SDK auto-wire `/.well-known/oauth-protected-resource`, `/.well-known/oauth-authorization-server`, `/authorize`, `/token`, and `/register` (dynamic client registration is enabled, since claude.ai registers itself rather than being given a static client id/secret) plus the bearer-auth middleware that gates `/mcp` — nothing to hand-roll there.

The provider is an OAuth-proxy: `mcp_server.py` is the Authorization Server as far as the MCP client (claude.ai) is concerned, but `authorize()` redirects the browser to Google, and the one custom route mcp_server.py adds — `@mcp.custom_route("/google/callback", ...)` (unauthenticated, since Google's redirect target can't carry a bearer token) — verifies Google's signed ID token (via `PyJWKClient` against Google's JWKS, checking `iss`/`aud`/`email_verified`), resolves the email to a Telegram `chat_id` via the `MCP_OIDC_USERS` env var (JSON `{"email": "chat_id"}`, or `"*"` for admin access to every user), and only then issues its own authorization code. Exchanging that code (`/token`) mints a self-signed HS256 JWT access token (`MCP_OIDC_JWT_SECRET`, 1-hour TTL) with the resolved `chat_id` as `sub`, plus an opaque refresh token.

Every `chat_id`-scoped tool in `mcp_server.py` is wrapped with `@_scoped` (applied under `@mcp.tool()`, so it still sees the original signature via `functools.wraps`), which calls `_check_access(chat_id)`: reads the current request's `AccessToken` via `mcp.server.auth.middleware.auth_context.get_access_token()` and raises `PermissionError` unless `token.subject == chat_id` or `token.subject == "*"` (admin). `list_users()` applies the same rule manually (no `chat_id` argument to decorate) by filtering its result set instead of raising. This is what stops an authenticated-but-non-admin caller from reading or writing another Telegram user's data — before this, any caller holding the shared token could pass any `chat_id` to any tool.

All OAuth state (pending Google logins keyed by a random `state`, issued authorization codes, refresh tokens) lives in the `GoogleOAuthProxyProvider` instance's memory only, matching the project's existing posture toward ephemeral runtime state (in-memory APScheduler jobs, the auto-generated ephemeral `MASTER_KEY` fallback) — a restart just forces callers to log in again, and nothing OAuth-related touches `state.json`/`bot_memory.db`.

Config additions in `mcp_remote.env` (gitignored): `GOOGLE_OIDC_CLIENT_ID` / `GOOGLE_OIDC_CLIENT_SECRET` (a Google Cloud Console OAuth Web-application client, redirect URI `https://<MCP_REMOTE_DOMAIN>/google/callback`), `MCP_OIDC_USERS`, `MCP_OIDC_JWT_SECRET` (generate like `MASTER_KEY`: `python3 -c "import secrets; print(secrets.token_hex(32))"`). `MCP_REMOTE_TOKEN` is gone. `MCP_REMOTE_DOMAIN` now does double duty: the `TransportSecuritySettings` DNS-rebinding `Host` check (as before) *and* the OIDC `issuer_url`/Google-callback base — it must still match the hostname nginx forwards as `Host: $host`.
